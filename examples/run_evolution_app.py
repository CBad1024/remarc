...
st.markdown("Choose a policy to run the Wright-Fisher evolutionary simulations, generate the 3D fitness landscape trajectory, and interact with the animated result.")
...
with col1:
    ...
    if pth_files:
        selected_from_list = st.selectbox(
            ...
            index=1 if "best_policy_chen_40gps_100st_1000rewsc_64b_100ep_e-3MR.pth" in pth_files else 0
        )
    ...
    pop_size = st.number_input("Population Size", min_value=100, max_value=1000000, value=10000, step=1000)
    gen_per_step = st.number_input("Generations per Step", min_value=1, max_value=5000, value=50, step=50)

with col2:
    st.subheader("Run Simulation")
    st.write("Clicking run will execute the Wright-Fisher simulation across 3 policies: your chosen Optimal Policy, a Random Policy, and the Best Single Drug policy.")
    
    run_btn = st.button("🚀 Run & Render Animation", use_container_width=True, type="primary")
...
if output_html_path.exists():
    ...
    st.caption("You can toggle between policies using the buttons in the player below, play/pause the trajectory, and rotate the 3D surface.")
    components.html(html_content, height=950, scrolling=True)
else:
    st.info("No generated simulation player found. Click run above to generate and display the interactive interface.")
