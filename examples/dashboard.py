...
DATASET_OPTIONS = {
    ...
    "Four-State": {
        "N": 2,
        "description": "Empirical four-state fitness landscapes (4 drugs, N=2).",
        "cli": "four_state",
    },
    "Three-State": {
        "N": 2,
        "description": "Empirical three-state fitness landscapes (3 genotypes).",
        "cli": "three_state",
    },
    "Synthetic": {
        "N": "any",
        "description": "Randomly generated landscapes with configurable N.",
        "cli": "synthetic",
    },
}
...
def copy_settings_callback(tab_id):
    ...
    widget_mapping = {
        ...
        "test_episodes": "te",
        "test_episode_length": "tel",
        "plot_shepherd": "shep",
        "shepherd_resolution": "shep_res",
...
def start_simulation(tab_id):
    ...
    cmd = [
        ...
        sim["hp"]["activation"],
        "--dataset",
        DATASET_OPTIONS[sim["hp"]["dataset"]]["cli"],
        "--ent-coef",
        str(sim["hp"].get("ent_coef", 0.05)),
        "--gamma",
        str(sim["hp"].get("gamma", 0.99)),
        "--gae-lambda",
        str(sim["hp"].get("gae_lambda", 0.95)),
        "--delta-multiplier",
        str(sim["hp"].get("delta_multiplier", 0.0)),
        "--episode-steps",
        str(sim["hp"].get("episode_steps", 20)),
        "--reward-scale",
        str(sim["hp"].get("reward_scale", 100.0)),
        "--landscape-amplification",
        str(sim["hp"].get("landscape_amplification", 1.0)),
        "--test-episodes",
        str(sim["hp"].get("test_episodes", 100)),
        "--test-episode-length",
        str(sim["hp"].get("test_episode_length", 100)),
    ]
    ...
    if mode_arg in ["wf_ls", "wf_ss"]:
        cmd += [
            ...
            str(sim["hp"]["pop_size"]),
            "--mutation-rate",
            str(sim["hp"]["mutation_rate"]),
            "--gen-per-step",
            str(sim["hp"]["gen_per_step"]),
        ]
    ...
    if signature:
        for folder in ["trajectories", "policies"]:
            fpath = (
                ...
                / "log"
                / folder
                / f"{signature}_live.{'csv' if folder == 'trajectories' else 'json'}"
            )
            if fpath.exists():
                fpath.unlink()
...
def update_logs_for_sim(tab_id):
    ...
    if sim["process"] and sim["process"].stdout is not None:
        ...
        if exit_code is not None:
            ...
            sim["process"] = None
            st.session_state.cooldown_until = (
                time.time() + st.session_state.cooldown_seconds
            )
            changed = True
            ...
            if exit_code == 0:
                ...
                st.toast(
                    f"❌ Simulation {tab_id + 1} Failed (Code: {exit_code})", icon="⚠️"
                )
            st.rerun()
    return changed
...
def plot_landscape_heatmap(tab_id):
    ...
    if sim["hp"]["dataset"] == "Chen et al.":
        ...
        st.info(
            "Synthetic landscape visualization coming soon (requires generating a sample for N={}).".format(
                sim["hp"]["n_mut"]
            )
        )
    ...
    if traj_file.exists():
        try:
            ...
            if not df.empty:
                ...
                ax.set_yticklabels(
                    [
                        bin(i)[2:].zfill(sim["hp"]["n_mut"])
                        for i in range(2 ** sim["hp"]["n_mut"])
                    ]
                )
                ax.set_xlabel("Time Step")
                ax.set_ylabel("Genotype")
                ax.set_title(
                    f"Tumor Mutation Path (Mean Fitness: {np.mean(ep_df['fitness']):.4f})"
                )
...
def plot_live_policy(tab_id):
    ...
    try:
        ...
        is_empirical = sim["hp"]["dataset"] in [
            "Chen et al.",
            "Four-State",
            "Three-State",
        ]
        if is_empirical:
            if sim["hp"]["dataset"] == "Chen et al.":
                ...
                amp = sim["hp"].get("landscape_amplification", 1.0)
                true_data = define_four_state_landscapes(
                    amplification=amp
                )  # Shape (4 drugs, 4 genotypes)
            ...
            axes[0].set_xticklabels(
                [
                    bin(i)[2:].zfill(int(np.log2(data["n_states"])))
            ...
            true_norm = (true_landscape - true_landscape.min()) / (
                true_landscape.max() - true_landscape.min()
            )
            q_norm = (q_values - q_values.min()) / (q_values.max() - q_values.min())
            diff = q_norm - true_norm

            im3 = axes[2].imshow(
                diff.T, aspect="auto", cmap="coolwarm", vmin=-1, vmax=1
            )
            ...
            st.caption(
                "💡 High correlation means the agent is learning the true fitness structure. Divergence may indicate strategic optimization beyond immediate fitness."
            )
        else:
...
def plot_simplex_section(tab_id):
    ...
    if is_three_state:
        st.caption(
            "2D simplex (x₀+x₁+x₂=1). Color shows which drug the policy selects at each population composition."
        )
    else:
        st.caption(
            "3D simplex (x₀+x₁+x₂+x₃=1) sliced along x₃. Color shows which drug the policy selects at each population composition."
        )
    ...
    if signature:
        ...
        if policy_path.exists():
            ...
            try:
                ...
                trained_fn = _load_ppo_policy_fn(
                    str(policy_path), state_dim=state_dim, n_actions=len(landscapes)
                )
                ...
                st.caption(
                    "Heatmap of normalized fitness under the drug chosen by the RL policy. Lower is better (darker). Red dashed lines indicate drug boundaries."
                )
                ...
                if sim["hp"].get("plot_shepherd", True):
                    ...
                    if (
                        "shepherd_fn" not in sim
                        or sim.get("shepherd_res_cache") != current_L
                    ):
                        with st.spinner(
                            f"Solving exact SHEPHERD MDP (L={current_L}) for visualization..."
                        ):
                            ...
                            landscape_objs = [
                                Landscape(2, sigma=0.0, ls=ls, g_min=g_min, g_max=g_max)
                            ...
                            cache_path = (
                                project_root / "log" / f"shepherd_L{current_L}.npz"
                            )
                            shepherd_mdp = ShepherdMDP.from_env(
                                dummy_env, L=current_L, discount=0.99
                            )
            ...
            st.info(
                "Trained policy not yet available. Will show after training completes."
            )
...
def plot_baseline_comparison(tab_id):
    ...
    try:
        ...
        if random_data:
            ...
            with cols[3]:
                improvement = (
                    (baseline_data["mean_fitness"] - learned_data["mean_fitness"])
                    / baseline_data["mean_fitness"]
                    * 100
                )
                st.metric(
        ...
        if all_vals:
            ...
            # Map trajectories
            learned_trajs = [
                [(v - global_min) / denom for v in t] for t in learned_trajs
            ]
            if random_trajs:
                random_trajs = [
                    [(v - global_min) / denom for v in t] for t in random_trajs
                ]
            if shepherd_trajs:
                shepherd_trajs = [
                    [(v - global_min) / denom for v in t] for t in shepherd_trajs
                ]
            ...
            if has_all_drugs:
                ...
                if "trajectories" in baseline_data:
                    baseline_data["trajectories"] = [
                        [(v - global_min) / denom for v in t]
                        for t in baseline_data["trajectories"]
                    ]
        ...
        learned_padded = np.array(
            [t + [np.nan] * (max_len - len(t)) for t in learned_trajs]
        )
        learned_mean = np.nanmean(learned_padded, axis=0)
        learned_std = np.nanstd(learned_padded, axis=0)
        ax.plot(
            timesteps, learned_mean, label="Learned Policy", color="blue", linewidth=2.5
        )
        ax.fill_between(
            timesteps,
        ...
        if show_greedy:
            if "greedy_trajs" not in sim or len(sim["greedy_trajs"][0]) < max_len:
                with st.spinner("Simulating Greedy Policy..."):
                    ...
                    if DATASET_OPTIONS.get(dataset, {}).get("cli") == "four_state":
                        ...
                        amp = sim["hp"].get("landscape_amplification", 1.0)
                        landscapes_array = define_four_state_landscapes(
                            amplification=amp
                        )
                        g_min_val, g_max_val = (
                        ...
                        landscape_objs = [
                            Landscape(
                                2, sigma=0.0, ls=ls, g_min=g_min_val, g_max=g_max_val
                            )
                            for ls in landscapes_array
                        ]
                        ...
                        landscape_objs = [
                            Landscape(
                                3, sigma=0.0, ls=ls, g_min=g_min_val, g_max=g_max_val
                            )
                            for ls in landscapes_array
                        ]
            ...
            if "greedy_trajs" in sim and sim["greedy_trajs"]:
                ...
                greedy_padded = np.array(
                    [t + [np.nan] * max(0, max_len - len(t)) for t in greedy_trajs]
                )
                ...
                ax.fill_between(
                    timesteps[:plot_len],
                    (greedy_mean - greedy_std)[:plot_len],
                    (greedy_mean + greedy_std)[:plot_len],
                    alpha=0.2,
                    color="#f1c40f",
                    zorder=9,
        ...
        if has_all_drugs:
            for drug_key, drug_trajs in baseline_data["all_drug_trajectories"].items():
                ...
                padded = np.array(
                    [t + [np.nan] * (max_len - len(t)) for t in drug_trajs]
                )
                ...
                ax.fill_between(
                    timesteps, d_mean - d_std, d_mean + d_std, alpha=0.05, color=color
                )
        else:
            ...
            if baseline_trajs:
                padded = np.array(
                    [t + [np.nan] * (max_len - len(t)) for t in baseline_trajs]
                )
                ...
                ax.plot(
                    timesteps,
                    b_mean,
                    label=f"Best Single Drug (#{best_drug})",
                    color="orange",
                    linewidth=2,
                    linestyle="--",
                )
        ...
        if random_data and random_trajs:
            random_padded = np.array(
                [t + [np.nan] * (max_len - len(t)) for t in random_trajs]
            )
        ...
        # SHEPHERD policy
        if shepherd_trajs:
            shepherd_padded = np.array(
                [t + [np.nan] * (max_len - len(t)) for t in shepherd_trajs]
            )
        ...
        st.caption(
            "💡 Lower fitness indicates better drug efficacy. Learned policy should show lower fitness (more effective) than single-drug baselines."
        )
        ...
        if "state_trajectories" in learned_data and learned_data["state_trajectories"]:
            ...
            try:
                ...
                if sim.get("selected_policy") and os.path.isabs(sim["selected_policy"]):
                    ...
                    mode_arg = MODES[sim["mode"]]
                    log_dir = (
                        "RL" if mode_arg in ["wf_ls", "wf_ss", "sswm"] else "sswm_dqn"
                    )
                    policy_path = (
                        project_root / "log" / log_dir / f"best_policy_{signature}.pth"
                    )
                ...
                if policy_path and policy_path.exists():
                    ...
                    trained_fn = _load_ppo_policy_fn(
                        str(policy_path), state_dim=4, n_actions=num_drugs
                    )
                else:
                    st.warning(
                        f"Policy file not found or signature missing. Signature: {signature}, Path: {policy_path}"
                    )
                ...
                st.caption(
                    "Heatmap of population state frequency during evaluation episodes, binned by small triangles."
                )
            except Exception as e:
                import traceback

                st.warning(
                    f"Could not generate population density plot: {e}\n\n```python\n{traceback.format_exc()}\n```"
                )
        else:
            st.info(
                "Population density data not found in baseline JSON. Re-run evaluation to generate it."
            )
    ...
    def status_fragment():
        ...
        if sim["running"]:
            ...
            if sim["exit_code"] == 0:
                ...
                st.error(f"❌ Failed (Exit Code: {sim['exit_code']})")
        else:
...
st.markdown(
    "Tinker with Evolutionary Dynamics and Reinforcement Learning right in your browser."
)
...
st.markdown(
    "<style>.stRadio > div { flex-direction: row; }</style>", unsafe_allow_html=True
)
active_tab = st.radio(
    "Select Simulation",
...
def render_tab_content(tab_id):
    ...
    with st.container():
        ...
        with cols_top[2]:
            sim["hp"]["epochs"] = st.number_input(
                "Epochs", 1, 1000, sim["hp"]["epochs"], key=f"epochs_{tab_id}"
            )
        with cols_top[3]:
            sim["hp"]["batch_size"] = st.selectbox(
                "Batch size",
                [16, 32, 64, 128, 256],
                index=[16, 32, 64, 128, 256].index(sim["hp"].get("batch_size", 128)),
                key=f"batch_{tab_id}",
            )

        with cols_top[4]:
            can_run = not sim["train"] or (
                sim["train"] and sim.get("signature", "").strip() != ""
            )
        ...
        with cols_top[6]:
            ...
            if has_sig:
                sim["starred"] = st.toggle(
                    "⭐ Star Run",
                    value=sim.get("starred", False),
                    key=f"star_run_{tab_id}",
                )
            else:
        ...
        with cols_top[7]:
            if st.button(
                "🗑️ CLEAR", key=f"clear_tab_btn_{tab_id}", use_container_width=True
            ):
                # Archive the old run if it wasn't starred
                if sim.get("signature") and not sim.get("starred", False):
                    archive_run(sim["signature"], False)
        ...
        with st.expander("📋 Copy Settings from another Tab"):
            ...
            with copy_cols[0]:
                st.selectbox(
                    "Source Tab",
                    [f"Sim {i + 1}" for i in range(NUM_TABS) if i != tab_id],
                    key=f"copy_source_{tab_id}",
                )

            with copy_cols[1]:
                st.button(
                    "Copy Settings",
                    key=f"copy_btn_{tab_id}",
                    use_container_width=True,
                    on_click=copy_settings_callback,
                    args=(tab_id,),
                )
    ...
    with col_env:
        ...
        sim["hp"]["n_mut"] = st.slider(
            "Number of mutations (N)", 1, 10, sim["hp"]["n_mut"], key=f"n_mut_{tab_id}"
        )

        # Only show Sigma for Synthetic datasets
        is_synthetic = sim["hp"]["dataset"] == "Synthetic"
        if is_synthetic:
            sim["hp"]["sigma"] = st.slider(
                "Sigma (Noise)",
                0.0,
                1.0,
                sim["hp"]["sigma"],
        ...
        sim["train"] = st.checkbox(
            "Enable Training", value=sim["train"], key=f"train_cb_{tab_id}"
        )
        ...
        if sim["train"]:
            ...
            policies = (
                sorted([f.name for f in path.glob("*.pth")]) if path.exists() else []
            )
            ...
            custom_policy_path = st.text_input(
                ...
                value=sim.get("selected_policy", "")
                if sim.get("selected_policy", "") not in policies
                else "",
    ...
    with col_regime:
        ...
        available_datasets = [
            name
            for name, meta in DATASET_OPTIONS.items()
            if meta["N"] == "any" or meta["N"] == sim["hp"]["n_mut"]
        ]
        ...
        sim["hp"]["dataset"] = st.selectbox(
            ...
            else available_datasets.index(dataset_key),
            key=f"dataset_sel_{tab_id}",
            help=str(DATASET_OPTIONS[dataset_key]["description"])
            if dataset_key in DATASET_OPTIONS
            else "",
        )

        if sim["hp"]["dataset"] == "Chen et al." and sim["hp"]["n_mut"] != 3:
            st.warning(
                "Chen dataset is strictly for N=3. Please adjust the mutation slider."
            )

        if sim["hp"]["dataset"] == "Four-State" and sim["hp"]["n_mut"] != 2:
            st.warning(
                "Four-State dataset is strictly for N=2. Please adjust the mutation slider."
            )
        ...
        sim["hp"]["activation"] = st.selectbox(
            ...
            index=[
                ...
                "elu",
                "gelu",
            ].index(sim["hp"]["activation"]),
            key=f"act_{tab_id}",
        )

        sim["hp"]["reward_clip"] = st.checkbox(
            "Enable Reward Clipping",
            value=sim["hp"].get("reward_clip", False),
            key=f"clip_{tab_id}",
            help="Clip rewards to [-5, 5] for stability",
        )

        if MODES[sim["mode"]] in ["wf_ls", "wf_ss"]:
            sim["hp"]["pop_size"] = st.number_input(
                "Population Size",
                100,
                1000000,
                sim["hp"]["pop_size"],
            ...
            sim["hp"]["mutation_rate"] = st.number_input(
                "Mutation Rate",
                0.0,
                1.0,
                sim["hp"]["mutation_rate"],
                format="%.1e",
                key=f"mut_{tab_id}",
            )
            sim["hp"]["gen_per_step"] = st.number_input(
                "Gens per Step",
                1,
                10000,
                sim["hp"]["gen_per_step"],
            ...
            sim["hp"]["ent_coef"] = st.number_input(
                "Entropy Coef.",
                0.0,
                1.0,
                sim["hp"].get("ent_coef", 0.05),
                step=0.01,
                format="%.4f",
                key=f"ent_{tab_id}",
                help="Higher = more exploration",
            )
            sim["hp"]["gamma"] = st.number_input(
                "Discount Factor (gamma)",
                0.0,
                1.0,
                sim["hp"].get("gamma", 0.99),
                step=0.01,
                format="%.4f",
                key=f"gamma_{tab_id}",
                help="Discount factor. Higher = longer horizon",
            )
            sim["hp"]["gae_lambda"] = st.number_input(
                "GAE Lambda",
                0.0,
                1.0,
                sim["hp"].get("gae_lambda", 0.95),
                step=0.01,
                format="%.4f",
                key=f"gae_{tab_id}",
                help="GAE lambda. Higher = higher variance, lower bias",
            )
            sim["hp"]["delta_multiplier"] = st.number_input(
                "Delta Bonus Multiplier",
                0.0,
                10.0,
                sim["hp"].get("delta_multiplier", 0.0),
                step=0.1,
                key=f"delta_{tab_id}",
                help="Rewards immediate fitness drops. Set to 0.0 to allow RL to sacrifice short-term fitness for long-term payoffs (avoids greedy trap).",
            )
            sim["hp"]["episode_steps"] = st.number_input(
                "Episode Steps",
                1,
                1000000,
                sim["hp"].get("episode_steps", 20),
                key=f"ep_steps_{tab_id}",
                help="Number of steps per episode",
            )
            sim["hp"]["reward_scale"] = st.number_input(
                "Reward Scale",
                0.1,
                10000.0,
                sim["hp"].get("reward_scale", 100.0),
                step=10.0,
                key=f"reward_scale_{tab_id}",
                help="Scale for rewards (recommended 100 for WF)",
            )
            sim["hp"]["random_start"] = st.checkbox(
                "Random Start",
                value=sim["hp"].get("random_start", True),
                key=f"rstart_{tab_id}",
                help="Start each episode from a random genotype instead of all-zeros",
            )
            sim["hp"]["stochastic"] = st.checkbox(
                "Stochastic (Multinomial Sampling)",
                value=sim["hp"].get("stochastic", True),
                key=f"stochastic_{tab_id}",
                help="ON = Wright-Fisher with genetic drift (multinomial sampling). OFF = Fokker-Planck deterministic mode (faster, no drift noise).",
            )

            st.markdown("##### Testing Parameters")
            sim["hp"]["test_episodes"] = st.number_input(
                "Test Episodes",
                1,
                10000,
                sim["hp"].get("test_episodes", 100),
                key=f"te_{tab_id}",
            )
            sim["hp"]["test_episode_length"] = st.number_input(
                "Test Episode Length",
                1,
                10000,
                sim["hp"].get("test_episode_length", 100),
                key=f"tel_{tab_id}",
            )
            sim["hp"]["plot_shepherd"] = st.checkbox(
                "Evaluate SHEPHERD Baseline (N<=3)",
                value=sim["hp"].get("plot_shepherd", False),
                key=f"shep_{tab_id}",
                help="Disable to save computation time",
            )
            if sim["hp"]["plot_shepherd"]:
                sim["hp"]["shepherd_resolution"] = st.number_input(
                    "SHEPHERD Lattice Resolution (L)",
                    min_value=2,
                    max_value=20,
                    value=sim["hp"].get("shepherd_resolution", 3),
                    key=f"shep_res_{tab_id}",
                    help="Number of grid points per axis in u-space. Default is 3.",
                )
    ...
    if sim["train"]:
        new_sig = sig_placeholder.text_input(
            "Run Signature", value=sim["signature"], key=f"sig_input_{tab_id}"
        )
        ...
        if not sim["signature"]:
            caption_placeholder.caption(
                "⚠️ :red[Run Signature is mandatory for training]"
            )
...
with st.sidebar:
    ...
    st.session_state.cooldown_seconds = st.number_input(
        "Cooldown (seconds)",
        min_value=0,
        value=st.session_state.cooldown_seconds,
        help="Wait time between simulations to prevent overheating.",
    )
    ...
    if st.button("Queue All Configured", use_container_width=True, type="primary"):
        for i in range(NUM_TABS):
            ...
            can_run = not s["train"] or (
                s["train"] and s.get("signature", "").strip() != ""
            )
...
st.divider()
st.caption(
    "REMARC - Reinforcement-learning based Evolutionary Markovian Resistance Control | support for concurrent executions"
)
