...
def compare_models(onnx_model_path, policy_path, steps=500, gen_per_step=1):
    ...
    print(f"Setting simulation switch interval to {gen_per_step} generations per step...")
    ...
    if match:
        ...
        print(f"Injecting explicit mutation rate {mutation_rate} from policy name into environment...")
        env.mutation_rate = mutation_rate
        env.mutation_matrix = env._build_mutation_matrix()
    else:
        print(f"Warning: Could not parse mutation rate from {policy_path}. Using environment default: {env.mutation_rate}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy", type=str, required=True, help="Original policy file path to extract parameters from")
