...
if "initialized" not in st.session_state:
    ...
    landscape_list = [
        ...
        for i in range(len(st.session_state.ls))
    ]
...
def build_3d_plot(current_frame, ls):
    ...
    fig.update_layout(
        ...
        scene=dict(
            ...
            yaxis=dict(
                title="Drug", tickvals=list(range(NUM_DRUGS)), ticktext=DRUG_NAMES
            ),
    ...
    if len(history) > 1:
        ...
        for i in range(1, len(history)):
            if history[i]["drug_idx"] != cur_drug:
                intervals.append(
                    (history[start_idx]["step"], history[i - 1]["step"], cur_drug)
                )
                start_idx = i
        ...
        for t0, t1, d_idx in intervals:
            ...
            if t1 - t0 > 0.5:
                fig.add_annotation(
                    ...
                    showarrow=False,
                    font=dict(size=10, color=DRUG_SOLID_COLORS[d_idx]),
                )
...
with col_side:
    ...
    st.info(
        f"Applying **{selected_drug_name}** will shift the fitness landscape and drive selection for different genotypes."
    )
