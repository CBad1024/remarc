...
def run_client(model_path, state_vector):
    ...
    drug_name = drug_names[best_drug_index] if best_drug_index < len(drug_names) else f"Drug {best_drug_index}"
    
    print(f"\n--> The ONNX model recommends applying: {drug_name}")
    return best_drug_index

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Lightweight ONNX Client for RL Policy")
    parser.add_argument("--model", type=str, default="rl_policy.onnx", help="Path to the .onnx file")
    parser.add_argument(
        "--state", 
        type=float, 
        nargs="+", 
