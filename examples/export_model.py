...
def export_and_test(
    ...
    policy = load_policy_for_export(
        policy_path, state_dim, n_actions, "relu", hidden_sizes, actor_sizes
    )
    ...
    try:
        ...
        print(
            "onnxruntime is not installed. Run `uv pip install onnx onnxruntime` to test the exported model."
        )
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--policy", type=str, required=True, help="Path to the .pth policy file"
    )
    parser.add_argument(
        "--out",
        type=str,
        default="rl_policy.onnx",
        help="Path for the output .onnx file",
    )
    parser.add_argument(
        "--state-dim", type=int, default=4, help="Dimension of state space"
    )
