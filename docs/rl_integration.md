...
`remarc` utilizes Reinforcement Learning (RL) to discover treatment scheduling policies that minimize population fitness over time.

## Frameworks Used

### Tianshou Integration (`remarc/agents/tianshou_agent.py`)
...
### Custom Learner (`remarc/agents/legacy_learner.py`)
